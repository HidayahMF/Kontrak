...
export const api = {
  ...
  create: (input: { nip: string; department?: string; startDate: string; endDate: string; contractNumber?: string }) => request<Contract>('/contracts', { method: 'POST', body: JSON.stringify(input) }),
  ...
  importExcel: async (file: File) => { const form = new FormData(); form.append('file', file); const response = await fetch(`${base}/contracts/import`, { method: 'POST', credentials: 'include', body: form }); const json = await response.json(); if (!response.ok) throw new Error(json.message ?? 'Import Excel gagal.'); return json.data as { total: number; imported: number; skipped: number; failed: number; results: { row: number; nip?: string; success: boolean; skipped?: boolean; message: string }[] }; },
